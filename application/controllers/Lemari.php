	...
	{	
		if($this->session->has_userdata("username")){
			...
			$menu["akses"] = $this->session->userdata("menu");			
 			$data["data_tabel"] = $this->LemariModel->getData();
			...
			$this->load->view('lemari/lemari_layout',$data);
...
// TAMBAH DATA =====================
	...
	{	
		if($this->session->has_userdata("username")){
			...
			$data["autonum"] = $this->LemariModel->getAutonumb();


			$this->load->view('header_admin');
			$this->load->view('sidemenu',$menu);
			$this->load->view('lemari/lemari_add',$data);
			$this->load->view('footer_admin');	
		}else{
			redirect(base_url().'Login');
		}
	...
	{	
		if($this->session->has_userdata("username")){
			...
			$save = $this->LemariModel->SimpanData($id,$nama);
			if($save==1){
...
// HAPUS DATA 	=============================
	...
	{	
		if($this->session->has_userdata("username")){
			$del = $this->LemariModel->HapusData($id);
			if($del==0){
...
//EDIT DATA ====================================
	...
	{	
		if($this->session->has_userdata("username")){
			...
			$data["data_tabel"] = (array)$this->LemariModel->getDataById($id);
	...
	{	
		if($this->session->has_userdata("username")){
			...
			$save = $this->LemariModel->UbahData($id,$nama);
			if($save==1){
				$data_save = array("msg"=>"Proses Simpan Berhasil !");		
			}else{
